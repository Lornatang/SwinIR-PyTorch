...
def define_loss(config: Any, device: torch.device) -> [nn.L1Loss, model.FeatureLoss, nn.BCEWithLogitsLoss]:
    ...
    if config["TRAIN"]["LOSSES"]["FEATURE_LOSS"]["NAME"] == "FeatureLoss":
        feature_criterion = model.FeatureLoss(
            config["TRAIN"]["LOSSES"]["FEATURE_LOSS"]["NET_CFG_NAME"],
            config["TRAIN"]["LOSSES"]["FEATURE_LOSS"]["BATCH_NORM"],
            config["TRAIN"]["LOSSES"]["FEATURE_LOSS"]["NUM_CLASSES"],
            config["TRAIN"]["LOSSES"]["FEATURE_LOSS"]["MODEL_WEIGHTS_PATH"],
            config["TRAIN"]["LOSSES"]["FEATURE_LOSS"]["FEATURE_NODES"],
            config["TRAIN"]["LOSSES"]["FEATURE_LOSS"]["FEATURE_NORMALIZE_MEAN"],
            config["TRAIN"]["LOSSES"]["FEATURE_LOSS"]["FEATURE_NORMALIZE_STD"],
        )
    else:
        raise NotImplementedError(f"Loss {config['TRAIN']['LOSSES']['FEATURE_LOSS']['NAME']} is not implemented.")
...
def train(
    ...
    feature_weight = torch.Tensor(config["TRAIN"]["LOSSES"]["FEATURE_LOSS"]["WEIGHT"]).to(device)
    ...
    while batch_data is not None:
        ...
        with amp.autocast():
            ...
            feature_loss = feature_criterion(sr, gt)
